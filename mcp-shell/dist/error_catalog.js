...
export function getErrorInfo(classification) {
    return catalog[classification] || catalog.unknown_error;
}
export function classifyByStderr(stderr) {
    const s = stderr.toLowerCase();
    if (s.includes("not found") || s.includes("cannot find path") || s.includes("не найден"))
        return "file_not_found";
    if (s.includes("access is denied") || s.includes("доступ запрещен") || s.includes("permission denied"))
        return "access_denied";
    if (s.includes("path") && (s.includes("invalid") || s.includes("incorrect") || s.includes("некоррект") || s.includes("ошиб")))
        return "path_issue";
    return null;
}
